...
import static org.domainobject.etl.engine.fnc.Default.AUTO;
import static org.domainobject.etl.engine.fnc.Default.THIS;
import org.domainobject.etl.engine.rt.CallInfo;
...
public final class StandardFunctionLibrary extends FunctionLibrary {
  ...
  @Param(uiName = "input", descr = "The string to extract a slice from", dfault = THIS)
  @Param(uiName = "from", descr = "The position of the first character of the slice")
  @Param(uiName = "to", descr = "The position of the last character of the slice", dfault = AUTO,
  ...
  @Param(uiName = "input", descr = "The value to map", dfault = THIS)
  @VarArgsParam(varArgs = {
      @VarArg(uiName = "match_candidate", description = "A value to compare the input value with"),
      @VarArg(uiName = "output", description = "The integer to output if the input value "
          + "was equal to the match candidate")})
  @Param(uiName = "default",
      descr = "The integer to output if the input did not match any of the " + "match candidates",
      dfault = AUTO, autoValue = "null")
  ...
  @VarArgsDefinition(factoryClass = StringInt.class, factoryMethod = "valueOf",
      userSignature = {String.class, int.class})
  public static int mapToInt(CallInfo callInfo, String input, StringInt[] varargs, int dfault) {
    for (StringInt arg : varargs) {
      if (input.equals(arg.getString())) {
        return arg.getInt();
      }
